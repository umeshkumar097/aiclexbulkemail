...
import { LiveEmailSending } from "@/components/dashboard/live-email-sending";
...
export function DashboardOverview({ stats }: DashboardOverviewProps) {
    ...
    // Use real stats if available, fallbacks to 0 for display
    const totalSent = stats?.totalSent?.toLocaleString() || "0";
    const openRate = stats?.openRate || "0.0";
    const clickRate = stats?.clickRate || "0.0";
    const activeLeads = stats?.activeLeadsCount?.toLocaleString() || "0";
    const hotLeads = stats?.hotLeads || [];
    ...
    return (
        ...
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    { title: "Total Emails Sent", value: totalSent, icon: Mail, trend: "+12.5%", trendUp: true },
                    { title: "Open Rate", value: `${openRate}%`, icon: Eye, trend: "+4.2%", trendUp: true },
                    { title: "Click Rate", value: `${clickRate}%`, icon: MousePointerClick, trend: "-1.1%", trendUp: false },
                    { title: "Active Leads", value: activeLeads, icon: Users, trend: "+8.4%", trendUp: true },
                ].map((stat, i) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <motion.div variants={item} className="flex flex-col gap-6">
                    ...
                    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex-1">
                        <h3 className="text-lg font-bold text-slate-900 mb-4">Top Leads</h3>
                                ...
                                <div key={i} className="flex items-center justify-between p-3 hover:bg-slate-50 rounded-lg transition-colors cursor-pointer group">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="overflow-hidden">
                                            <p className="text-sm font-semibold text-slate-900 truncate max-w-[120px]">{lead.name || lead.email}</p>
                                            <p className="text-xs text-slate-500 truncate max-w-[120px]">{lead.campaign?.name}</p>
                                        </div>
                                    </div>
